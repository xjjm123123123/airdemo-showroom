...
const Efficiency: React.FC = () => {
  return (
    <div className="flex-1 p-6 lg:p-16 overflow-y-auto overflow-x-hidden no-scrollbar bg-transparent animate-fadeIn relative h-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
          {EFFICIENCY_TOOLS.map((tool) => (
            <div key={tool.id} className="bg-black/40 backdrop-blur-md rounded-[var(--radius-lg)] border border-white/10 p-6 lg:p-8 hover:border-white/30 hover:bg-black/50 hover:shadow-2xl transition-all duration-300 group">
              <div className="flex items-start justify-between gap-5 mb-6 min-w-0">
                <div className="flex items-center gap-4 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <a href={tool.url} target="_blank" rel="noreferrer" className="text-sm font-medium text-white/40 hover:text-blue-400 truncate block mt-1 transition-colors max-w-full">
                      {tool.url}
                    </a>
                  </div>
                </div>
